...
abstract class AbstractNormalPosCircTouchPacker(highEffort: Boolean, lowerBound: Int) : GenericBinPacker<SegmentContainer>(highEffort, lowerBound) {
    ...
    override fun packIntoContainer(item: Box, container: SegmentContainer): Pair<PlacedBox, Double>? {
        ...
        //Logger.log("Found fit for box $box, total circumference: ${box.circumference}, touched: ${bestFit.third}")
    ...
    private fun findBestFit(box: Box, container: SegmentContainer) : Triple<Int, Int, Double>? {
    ...
    private fun evaluateFit(box: Box, container: SegmentContainer, x: Int, y: Int) : Double {
